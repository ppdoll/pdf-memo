...
- **Phase 1** PDF 가져오기·문서 목록·뷰어·휴지통 (완료), 펜·형광펜·마커·지우개·Undo/Redo (완료), 필기 포함 PDF 내보내기 (완료), 백업 zip·복원 (완료), 텍스트 메모·스티키 노트 (완료), 스티커·사용자 이미지 (완료), 마크다운·텍스트 가져오기 (완료), 이미지 가져오기 (완료), JSON 트리 뷰어·폴더 아이콘 (완료) ← 현재, 이어서 PWA 설치 배너·도형
