...
| PDF 쓰기   | `pdf-lib` + `fontkit` 2      | 브라우저에서 PDF에 벡터 패스·이미지·텍스트 삽입. **한글 텍스트 내보내기에는 CJK 폰트 임베드 필수**(표준 14폰트는 한글 미지원) → Pretendard(OFL) CFF 서브셋 임베드. 구형 `@pdf-lib/fontkit`은 FDArray에 FontMatrix가 있는 CID-keyed CFF를 못 읽어(Unknown operator 3079) fontkit 2 + 얇은 어댑터(`features/text/pdfFontkit.ts`)를 쓴다 |
...
- [x] 필기(2026-09-22): 펜·형광펜·마커·지우개·손, 압력, 손바닥 무시, 손가락 이동·핀치 확대, Undo/Redo, 자동 저장 표시
...
- [x] 이미지 가져오기(2026-09-22): JPEG·PNG·WebP·GIF·BMP·AVIF를 브라우저에서 디코딩(EXIF 회전 반영)해 긴 변 2500px로 줄이고 불투명이면 JPEG, 투명이면 PNG로 재인코딩한 뒤 한 장 = 한 페이지 PDF로 만든다(`features/import/images/`). 페이지는 이미지 비율 그대로, 짧은 변을 A4 폭(595.28pt)에 맞춰 도구 굵기가 문서와 비슷하게 보이게 한다(가로 사진은 가로 페이지). 2장 이상을 한 번에 넣으면 "각각 문서로 / 한 문서로 묶기" 대화상자(제목 "첫 이름 외 N장"). HEIC/HEIF는 브라우저가 못 열어 "높은 호환성"·JPEG 내보내기 안내
